...
def number_decider(field):
    ...
    for part in field:
        ...
        for lett in segm_num[7]:
            if(lett not in segm_num[1]):
                for number in part:
                    lisst = list(number)
                    if((lett in lisst) and (segm_num[4][0] in lisst) and (segm_num[4][1] in lisst) and (segm_num[4][2] in lisst) and (segm_num[4][3] in lisst) and (len(lisst) == 6)):
                        segm_num[9] = lisst
                        numbers[9] = number
                        break
        for number in part:
            lisst = list(number)
            if((segm_num[7][0] in lisst) and (segm_num[7][1] in lisst) and (segm_num[7][2] in lisst) and (numbers[9] != number) and (len(lisst) == 6)):
                segm_num[0] = lisst
                numbers[0] = number
                break
        for number in part:
            if((number != numbers[9]) and (number != numbers[0]) and (len(list(number)) == 6)):
                segm_num[6] = list(number)
                numbers[6] = number
                break
        for number in part:
            lisst = list(number)
            for letter in possibilities:
                if(letter not in segm_num[6]):
                    if((letter not in lisst) and (len(lisst) == 5)):
                        segm_num[5] = lisst
                        numbers[5] = number
                        break
        for number in part:
            lisst = list(number)
            if((segm_num[1][0] in lisst) and (segm_num[1][1] in lisst) and (len(lisst) == 5) and (number != numbers[5])):
                segm_num[3] = lisst
                numbers[3] = number
                break
        for number in part:
            lisst = list(number)
            if((number not in numbers) and (len(lisst) == 5)):
                segm_num[2] = lisst
                numbers[2] = number
                break
        ...
        for number in part:
            ...
            if((vertical is False) and (number == "|")):
                vertical = True

        for numb in result[0]:
            for value in range(len(numbers)):
                print(value)
                if(numbers[value] == numb):
                    result[1].append(value)
        print(numbers)
        print(result)


def convert_to_string(part):
    num = [[],[],[],[],[]]
    for number in part:
        if(len(number) == 2):
            num[0].append([" .... "])
    ...
    for i in range(len(num[2])):
        if (i == len(num[2]) - 1):
            ...
            print(str(num[2][i]) + " ", end="")
    for i in range(len(num[3])):
        if (i == len(num[3]) - 1):
            print(str(num[3][i]) + " ")
        else:
            print(str(num[3][i]) + " ", end="")
    for i in range(len(num[3])):
        if (i == len(num[3]) - 1):
            print(str(num[3][i]) + " ")
        else:
    ...
    for i in range(len(num[4])):
        if (i == len(num[4]) - 1):
            ...
            print(str(num[4][i]) + " ", end="")


nums = num_count(initial)
#print("Ones: " + str(nums[0]))
#print("Fours: " + str(nums[1]))
#print("Sevens: " + str(nums[2]))
#print("Eights: " + str(nums[3]))
#print("Sum: " + str(nums[0]+nums[1]+nums[2]+nums[3]))

#convert_to_string(showcase[0])

number_decider(showcase)