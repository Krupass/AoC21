...
showcase = [16,1,2,0,4,2,7,1,2,14]

tisic = 500500
tisicsto = 605550
tisicdveste = 720600
tisictrista = 845650
tisicctyrista = 980700
tisicpetset = 1125750
tisicsetset = 1280800
tisicsedmset = 1445850
tisicosmset = 1620900
tisicdevetset = 1805950
...
def smallest_part(field):
    small = [0,9999999999999999999999]
    for part in field:
        if(part[1] < small[1]):
            small = part
...
def allign_point(field):
    fuels = []
    for i in range(smallest(field), biggest(field)):
        fuel = 0
        for part in field:
            if(part > i):
                fuel = fuel + (part - i)
    ...
    return smallest_part(fuels)

def crab_fuel_usage(positions):
    fuel = 0
    if(positions < 1000):
        ...
        fuel = tisic
        for i in range(1001, positions + 1):
            fuel = fuel + i
    elif (positions < 1200):
        fuel = tisicsto
        for i in range(1101, positions + 1):
            fuel = fuel + i
    elif (positions < 1300):
        fuel = tisicdveste
        for i in range(1201, positions + 1):
            fuel = fuel + i
    elif (positions < 1400):
        fuel = tisictrista
        for i in range(1301, positions + 1):
            fuel = fuel + i

    elif (positions < 1500):
        fuel = tisicctyrista
        for i in range(1401, positions + 1):
            fuel = fuel + i

    elif (positions < 1600):
        fuel = tisicpetset
        for i in range(1501, positions + 1):
            fuel = fuel + i

    elif (positions < 1700):
        fuel = tisicsetset
        for i in range(1601, positions + 1):
            fuel = fuel + i

    elif (positions < 1800):
        fuel = tisicsedmset
        for i in range(1701, positions + 1):
            fuel = fuel + i

    elif (positions < 1900):
        fuel = tisicosmset
        for i in range(1801, positions + 1):
            fuel = fuel + i

    elif (positions < 2000):
        fuel = tisicdevetset
        for i in range(1901, positions + 1):
            fuel = fuel + i

    return fuel

def crab_point(field):
    fuels = []
    bigg = biggest(field)
    last = 0
    roun = 0
    for i in range(smallest(field), bigg):
        fuel = 0
        for part in field:
            if (part > i):
                fuel = fuel + crab_fuel_usage(part-i)
            else:
                fuel = fuel + crab_fuel_usage(i-part)
            #print("Position: " +str(i) + ", Fuel: " + str(fuel))
        fuels.append([i, fuel])
        roun = round(((1-(bigg-i)/bigg)*100))
        if(roun > last):
            last = roun
            print("Crab field progress: " + str(last) + " %")

    return smallest_part(fuels)

allign = allign_point(initial)
crab = crab_point(initial)
print("Position: " + str(allign[0]) + ", Fuel used: " + str(allign[1]))
#print(str(crab_fuel_usage(1584)))
print("Position: " + str(crab[0]) + ", Fuel used: " + str(crab[1]))
